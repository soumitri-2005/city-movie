...
export default function ImageTrail({ items = [], variant = 1 }) {
  ...
  return (
    <div
      className="w-full h-full relative rounded-lg bg-transparent overflow-visible"
      ref={containerRef}
    >
      {items.map((url, i) => (
        <div
          className="content__img w-[190px] aspect-[1.1] border-2 border-[var(--gold-color)]  rounded-[15px] absolute top-0 left-0 opacity-0 overflow-hidden [will-change:transform,filter]"
          key={i}
        >
          <div
      ...
      <div className="relative h-full w-full flex items-center flex-col justify-center">
        {/* Background Image */}
        <div className="absolute inset-0 h-full w-full object-cover px-5 ">
          <img
            src={heroImage}
            alt="hero-image"
            className="h-full w-full object-cover rounded-4xl brightness-[65%]"
          />
        </div>
        ...
        <h1
          ref={msgRef}
          className="text-[1.5rem] sm:text-[4rem] md:text-[6rem] lg:text-[9.5rem] font-[400] text-[var(--gold-color)] drop-shadow-2xl tracking-[0.5rem] z-[100] leading-tight"
        >
          City Movie
        </h1>
        ...
        <p
          ref={paraRef}
          className="formal-font-text text-[0.5rem] sm:text-[0.7rem] md:text-[1rem] lg:text-[1.4rem] text-yellow-300 drop-shadow-lg tracking-[3px] z-[90]"
        >
          Explore Our Website.
        </p>
      </div>
